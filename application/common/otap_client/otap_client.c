...
#endif
#if defined (SOTA_ENABLED)
    uint8_t                     imgCompatibilityList[gOtap_ImageCompatibilityListMaxSize_c];    /*!< Compatibility list */
#endif
...
{
    OTAP_CLIENT_LOG("%s\r\n", __FUNCTION__);
    bool_t serverWrittenCcdOldValue = otapClientData.serverWrittenCccd;
    ...
    if (handle == (uint16_t)cccd_otap_control_point)
    {
        ...
        case mOtapClientStateDownloadingImage_c:
            /* Continue the download if already in progess */
            if (!serverWrittenCcdOldValue)
            {
...
{
    OTAP_CLIENT_LOG("%s Ota state = 0x%x\r\n", __FUNCTION__, otapClientData.state);

    TMR_StopTimer(mTransferStatusCheckTmrId);
    switch (otapClientData.state)
    {
    case mOtapClientStateIdle_c:
...
/*! *********************************************************************************
* \brief        Reset the OTA client state machine
*
********************************************************************************** */
void OtapClient_ResetOtaClient ()
{
    OTAP_CLIENT_LOG("%s\r\n", __FUNCTION__);
...
/*! *********************************************************************************
* \brief        Returns the download state of the Otap Client
*
*\return        TRUE - If a download is in progress, FALSE otherwise
********************************************************************************** */
bool_t OtapClient_GetDownloadState()
{
    return (otapClientData.state == mOtapClientStateDownloadingImage_c);
}
...
static void OtapClient_SendImageBlockReq_cb(uint32_t param)
{
    ...
    if (gBleSuccess_c == bleResult)
    {
        ...
        (void)TMR_StartIntervalTimer(mTransferStatusCheckTmrId,
                                     gOTAPTransferCheckIntervalMs,
                                     OtapClient_TimerCb,
                                     NULL);
    }
    else
...
static void OtapClient_HandleDataChunk (deviceId_t deviceId, uint16_t length, uint8_t* pData)
{
    ...
    bleResult_t     bleResult;
    otapStatus_t    otapStatus = gOtapStatusSuccess_c;
    OTA_DBG_LOG("length=%d, data=%x", length, pData);
...
#if defined (SOTA_ENABLED)
    static uint32_t currentImgCompatibilityListLen = 0;
#endif
...
#if defined (SOTA_ENABLED)
            currentImgCompatibilityListLen = 0;
#endif
            ...
            else
            {
                ...
                else
                {
                    ...
                    case (uint16_t)gBleOtaSubElemTagIdUpgradeImage_c:
                        ...
                        if (currentImgElemRcvdLen == sizeof(subElementHeader_t))
                        {
                            otaResult_t otaStartImageResult = gOtaSuccess_c;
#if defined (SOTA_ENABLED)
                            uint32_t blobSectionSize;
                            uint16_t blobId;
                            FLib_MemCpy(&blobId, otapClientData.imgId, sizeof(uint16_t));
                            if (Sota_GetBlobSectionSize(blobId, &blobSectionSize))
                            {
                                otaStartImageResult = OTA_StartImageWithMaxSize(subElemHdr.dataLen, blobSectionSize);
                            }
                            else
#endif
                            {
                        ...
                        otapClientData.imgComputedCrc = OTA_CrcCompute (pData,
                                                                        ...
                                                                        otapClientData.imgComputedCrc);
                        if (gOtaSuccess_c != OTA_PushImageChunk (pData, (uint16_t)elementChunkLength, NULL, NULL))
                        {
                            otapStatus = gOtapStatusImageStorageError_c;
                            break;
                        }
                    ...
                    case (uint16_t)gBleOtaSubElemTagIdSectorBitmap_c:
                        APP_DEBUG_TRACE("TAG Id Sector Bitmap\r\n");
                    ...
                    case (uint16_t)gBleOtaSubElemTagIdImageFileCrc_c:
                        APP_DEBUG_TRACE("gBleOtaSubElemTagIdImageFileCrc_c\r\n");
                        /* Immediately after receiving the header check if the sub-element length is valid. */
                        if (currentImgElemRcvdLen == sizeof(subElementHeader_t))
                        {
                        ...
                        break;
#if defined (SOTA_ENABLED)
                    case gBleOtaSubElemTagIdCompatibilityList_c:
                        APP_DEBUG_TRACE("Handle compatibility list\r\n");
                        if (currentImgElemRcvdLen == sizeof(subElementHeader_t))
                        {
                            uint32_t compatibilityListLen = subElemHdr.dataLen;
                            bool_t comptListLenOk = FALSE;
                            uint8_t sota_blob_nb = Sota_GetNumberOfBlobs();
                            uint32_t residualLen = (elementChunkLength >= ((uint16_t)sizeof(otapClientData.imgCompatibilityList) - currentImgCompatibilityListLen))?
                                                   (sizeof(otapClientData.imgCompatibilityList) - currentImgCompatibilityListLen):
                                                   elementChunkLength;
                            /* Not enough data to complete the compatibility list. */
                            FLib_MemCpy((uint8_t*)(otapClientData.imgCompatibilityList) + currentImgCompatibilityListLen,
                                        pData,
                                        residualLen);
                            currentImgCompatibilityListLen += residualLen;

                            if ((compatibilityListLen % sizeof(sotaCompatibilityListElem_t) == 0)
                                && (compatibilityListLen <= sizeof (otapClientData.imgCompatibilityList))
                                && (compatibilityListLen >= currentImgCompatibilityListLen))
                            {
                                comptListLenOk = TRUE;
                                compatibilityListLen = compatibilityListLen/sizeof(sotaCompatibilityListElem_t);
                            }
                            /* Check the compatibility list len */
                            if (!comptListLenOk
                                || ((sota_blob_nb - 1) != compatibilityListLen))
                            {
                                /* The compatibility list length is invalid, set an error status and reset
                                 * the image file download process. */
                                APP_DEBUG_TRACE("Invalid length (%d) - %d %d\r\n",__LINE__,
                                        compatibilityListLen,
                                        sota_blob_nb);
                                otapStatus = gOtapStatusInvalidCompatibilityList_c;
                                break;
                            }
                            if (currentImgCompatibilityListLen/sizeof(sotaCompatibilityListElem_t) == compatibilityListLen)
                            {
                                uint16_t blobId;
                                FLib_MemCpy(&blobId, otapClientData.imgId, sizeof(blobId));
                                if (!Sota_CheckCompatibilityList(blobId, &otapClientData.imgCompatibilityList[0], compatibilityListLen))
                                {
                                    otapStatus = gOtapStatusInvalidCompatibilityList_c;
                                    break;
                                }
...
#endif
                    default:
                        APP_DEBUG_TRACE("Unknown TAG ID: %x\r\n", subElemHdr.tagId);
...
#else
                if (gOtaSuccess_c != OTA_CommitImage(NULL))
#endif
                {
                    otapStatus = gOtapStatusImageStorageError_c;
                }
...
#if gImageAuthenticate_c
                    if(OTA_ImageAuthenticate() == gOtaImageAuthFail_c)
                    {
                        otapStatus = gOtapStatusFailedImageIntegrityCheck_c;
                    }
                    else
#endif
                    {
...
static void OtapClient_HandleErrorNotification (deviceId_t deviceId, uint16_t length, uint8_t* pValue)
{
    ...
    if (length == cmdIdToCmdLengthTable[gOtapCmdIdErrorNotification_c])
    {
        ...
        if (pRemoteCmd->cmd.errNotif.errStatus < gOtapNumberOfStatuses_c)
        {
            ...
            OTA_DBG_LOG("Disconnect and restart");
            (void)Gap_Disconnect (deviceId);
        }
        else
        {
...
static void OtapClient_ContinueImageDownload (deviceId_t deviceId)
{
    ...
    if (bValidState)
    {
        ...
        case mOtapClientStateDownloadingImage_c:
            ...
            break;
        case mOtapClientStateImageDownloadComplete_c:
            OTA_DBG_LOG("Download complete 2");
            /* Stop timer */
            (void)TMR_StopTimer(mTransferStatusCheckTmrId);
...
static bool_t OtapClient_IsRemoteImageNewer (uint8_t* pRemoteImgId, uint8_t* pRemoteImgVer)
{
    bool_t      bIsNewer = FALSE;

    do {

#ifdef SOTA_ENABLED
        /* Try to get the blob version */
        uint16_t blobId = 0;
        uint32_t blobVersion =0;
        FLib_MemCpy(&blobId, pRemoteImgId, sizeof(uint16_t));

        if (Sota_GetBlobVersion(blobId,&blobVersion))
        {
            uint32_t remoteBlobVersion = 0;
            FLib_MemCpy(&remoteBlobVersion, pRemoteImgVer+sizeof(uint32_t), sizeof(uint32_t));
            /* Check that the blob version is newer */
            if (!Sota_CompareBlobUpdateVersion(blobVersion, remoteBlobVersion))
                break;
        }
        else
#endif
        {
            ...
            /* Check Hardware Id */
            if (!FLib_MemCmp((void*)(&(pRemoteImgVer[4])),
                             (const void*)(&(otapClientData.currentImgVer[4])),
                             3))
                break;
            ...
            /* Check Build Version */
            remoteBuildVer = (uint32_t)pRemoteImgVer[0] +
                ((uint32_t)(pRemoteImgVer[1]) << 8) +
                ((uint32_t)(pRemoteImgVer[2]) << 16);
            localeBuildVer = (uint32_t)otapClientData.currentImgVer[0] +
                ((uint32_t)(otapClientData.currentImgVer[1]) << 8) +
                ((uint32_t)(otapClientData.currentImgVer[2]) << 16);
...
static bleResult_t OtapClient_SendImageRequest(void)
{
    ...
    if (otapClientData.currentPos < otapClientData.imgSize)
    {
        ...
        if (bAbortOTA == FALSE)
        {
...
#if (gOtaErasePolicy_c == gOtaEraseBeforeChunkReq_c)
            ...
            {
                OTA_MakeHeadRoomForNextBlock(maxBlockSize,

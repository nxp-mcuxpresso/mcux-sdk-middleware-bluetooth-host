...
extern int BOARD_Get_BLE_MAC_Id(uint8_t mac_addr[gcBleDeviceAddressSize_c]);
    ...
    else
    {
        ...
        if(TRUE == bFsciBleGapEnabled)
        {
            ...
            switch(pClientPacket->structured.header.opCode)
            {
                ...
                case gBleCtrlCmdConfigureAdvCodingSchemeOpCode_c:
                    {
        ...
        defined(CPU_MKW35A512VFP4) || defined(CPU_MKW35Z512VHT4) || defined(CPU_MKW36A512VFP4) || \
        defined(CPU_MKW36A512VHT4) || defined(CPU_MKW36Z512VFP4) || defined(CPU_MKW36Z512VHT4))
                case gBleCtrlCmdSetScanDupFiltModeOpCode_c:
                    {
                        uint8_t mode;

                        fsciBleGetUint8ValueFromBufferSigned(mode, pBuffer);

                        fsciBleGapCallApiFunction(Controller_SetScanDupFiltMode(mode));
                    }
                    break;
#endif
...
static bleResult_t fsciWritePublicDeviceAddress(bleDeviceAddress_t bdAddress)
{
    ...
    bleDeviceAddress_t currentAddr, currentAddrReversed;

    BOARD_Get_BLE_MAC_Id(currentAddr);
    FLib_MemCpyReverseOrder(currentAddrReversed, currentAddr, sizeof(bleDeviceAddress_t));

    if (!FLib_MemCmp(bdAddress, currentAddrReversed, sizeof(bleDeviceAddress_t)))
    {
        buf[0] = buf[1] = 0;
        FLib_MemCpy(&buf[2], bdAddress, sizeof(bleDeviceAddress_t));
        if (psector_WriteBleMacAddress(&buf[0]) < 0)
        {
        	retStatus = gBleUnexpectedError_c;
        }
    }
